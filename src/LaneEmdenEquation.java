...
public class LaneEmdenEquation extends Equation {
	...
	// Use this method to define the 2nd order ODE needed in the numerical
	// calculation
	// This is the Lane-Emden Equation which will be called by the Euler,
	// Runge-Kutta, etc calculation classes
	...
	public Parameters getResult(BigDecimal h, Parameters p) {
		...
		// Y'' = Y' + h*(-2(Y'/t) - Y^n)
		// Note that BigDecimal cannot do fractional exponents - if y is very small
		// Math.pow will
		// error with NaN
		y = p.y.doubleValue();
		...
		if (n < 0.0) {
			pNew.m = yptemp.add(ytemp, MathContext.DECIMAL128).multiply(h, MathContext.DECIMAL128);
		} else {
	...
	// pNew.yprime = roundToSignificantFigures(p.yprime + h*(-2.0D*(p.yprime/p.t) -
	// Math.pow(p.y, n)), 12);
